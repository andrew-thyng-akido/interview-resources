...
export function CoffeeOrderForm({ customerName }: CoffeeOrderFormProps) {
  // Set size state to medium
  const [size, setSize] = useState("medium")
  const [milk, setMilk] = useState("whole")
  const [sugar, setSugar] = useState(0) // suger state for suger amount
  ...
  const [error, setError] = useState("") // TODO: fix this
  ...
  // This useEffect runs when customerName changes
  useEffect(() => {
    if (customerName !== currentCustomer) {
      ...
      setBeanName("") // This line of code sets the bean name variable to an empty string value which represents no bean being selected
  ...
  // Calculate price based on size only
  useEffect(() => {
    let price = 0
  ...
  useEffect(() => {
    ...
    fetch("/api/beans")
      ...
      .catch(err => {
        setError("Failed to load beans") // damn API is broken again
        setIsLoadingBeans(false)
      })
  ...
  const handleSizeChange = (newSize: string) => {
    setSize(newSize)
    // setPrice(newSize === "large" ? 5 : 4) // old pricing logic
  }
  ...
  const handleSubmit = () => {
    console.log("debugging submit function")
